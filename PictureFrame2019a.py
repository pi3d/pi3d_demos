...
#####################################################
PIC_DIR = '/home/pi/Pictures/Fnatenbilder/' #'textures'
...
def get_files(dt_from=None, dt_to=None):
  ...
  for root, _dirnames, filenames in os.walk(PIC_DIR):
      for filename in filenames:
          ...
          if ext in extensions and not '.AppleDouble' in root and not filename.startswith('.') and not ".sync" in root:
...
try:
  ...
  client = mqtt.Client()
  client.username_pw_set("htwypupm", "FznINUvtiijZ") # replace with your own id
  client.connect("m23.cloudmqtt.com", 17905, 60) # replace with your own server
  client.loop_start()
  ...
  if nFi > 0:
    if tm > nexttm and not paused: # this must run first iteration of loop
      ...
      if pic_num >= nFi:
        ...
        pic_num = 0
      ###########################################################
      # vary dwell time according to file name along the lines '12_abcd1a.jpeg' => 12s
      fname = iFiles[pic_num]
      if fname[0].isnumeric() and '_' in fname:
        dwell = int(fname.split('_')[0])
        nexttm += (dwell - time_delay) # nexttm already had time_delay added above so subtract it
      ###########################################################
